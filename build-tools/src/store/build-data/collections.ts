...
export async function createIndexes(db: Db): Promise<void> {
    // ── build_meta ──────────────────────────────────────────────────────────
    await db.collection(COLLECTIONS.META).createIndex(
        { domain: 1, version: 1 },
        { unique: true, name: "uq_domain_version" },
    );

    // ── build_docs ──────────────────────────────────────────────────────────
    await db.collection(COLLECTIONS.DOCS).createIndex(
        { domain: 1, version: 1, slug: 1 },
        { unique: true, name: "uq_domain_version_slug" },
    );
    await db.collection(COLLECTIONS.DOCS).createIndex(
        { domain: 1, version: 1, order: 1 },
        { name: "idx_domain_version_order" },
    );

    // ── build_flows ─────────────────────────────────────────────────────────
    await db.collection(COLLECTIONS.FLOWS).createIndex(
        { domain: 1, version: 1, flowId: 1 },
        { unique: true, name: "uq_domain_version_flowId" },
    );
    await db.collection(COLLECTIONS.FLOWS).createIndex(
    ...
    // ── build_validations ───────────────────────────────────────────────────
    await db.collection(COLLECTIONS.VALIDATIONS).createIndex(
        { domain: 1, version: 1 },
        { unique: true, name: "uq_domain_version" },
    );

    // ── build_changelog ─────────────────────────────────────────────────────
    await db.collection(COLLECTIONS.CHANGELOG).createIndex(
        { domain: 1, fromVersion: 1, toVersion: 1 },
        { unique: true, name: "uq_domain_from_to" },
    );
    await db.collection(COLLECTIONS.CHANGELOG).createIndex(
        { domain: 1, generatedAt: -1 },
        { name: "idx_domain_generatedAt_desc" },
    );
}
